...
class Block extends BlockHeader implements IBlock{
    ...
    // 추가할 블록을 찾으면 네트워크에 브로드 캐스트하고 다른 네트워크들은 내 체인과 블록을 받는다.
    // 블록 검증과 체인 검증했을 때 긴 체인이 있으면 더 긴 체인이 정답
    ...
    static getDifficulty(_newBlock : Block, _adjustmentBlock : Block, _previousBlock : Block) : number {
        // 실제로는 2016개 이전 블록의 생성 시간을 보고 난이도 조절을 진행. 여기서는 10개 이전으로 확인
        if(_newBlock.height <= 0) throw new Error('높이가 0이 들어옴');
        if(_newBlock.height < 10) return 0;
        if(_newBlock.height < 21) return 1;
        // 블록 높이가 20 이하일 경우 체크 안함
        // 블록 높이가 10의 배수가 아닐 경우 이전 블록 난이도로 설정
        // 블록당 10분이 생성 목표 시간
